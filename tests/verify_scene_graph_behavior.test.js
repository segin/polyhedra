...
describe('Scene Graph Behavior Verification', () => {
    ...
    test('handles reordering', () => {
        const obj1 = { uuid: '1', name: '1', visible: true, geometry: { type: 'Box' }, position: {x:0,y:0,z:0, toFixed:()=>'0'} };
        const obj2 = { uuid: '2', name: '2', visible: true, geometry: { type: 'Box' }, position: {x:0,y:0,z:0, toFixed:()=>'0'} };

        app.objects.push(obj1, obj2);
        app.updateSceneGraph();

        expect(app.objectsList.children[0].querySelector('.object-name').textContent).toBe('1');
        expect(app.objectsList.children[1].querySelector('.object-name').textContent).toBe('2');

        // Swap
        app.objects = [obj2, obj1];
        app.updateSceneGraph();

        expect(app.objectsList.children[0].querySelector('.object-name').textContent).toBe('2');
        expect(app.objectsList.children[1].querySelector('.object-name').textContent).toBe('1');
    });
});
