...
async function main() {
  ...
  try {
    ...
    console.log(
      `[BENCHMARK] Serialization (100 segments): ${time100.toFixed(2)}ms`,
    );

    // Heavy load
    const time500 = await runBenchmark(500);
    console.log(
      `[BENCHMARK] Serialization (500 segments): ${time500.toFixed(2)}ms`,
    );
