...
describe.skip("Serialization Optimization Benchmark", () => {
  ...
  test("Benchmark: Full serialization pipeline", () => {
    const boxCount = 50;
    console.log(
      `[BENCHMARK] Creating scene with ${boxCount} high-poly boxes...`,
    );
    ...
    console.log(
      `[BENCHMARK] Naive JSON size: ${(workerResultNaive.json.length / 1024 / 1024).toFixed(2)} MB`,
    );
    console.log(
      `[BENCHMARK] Naive Buffers count: ${workerResultNaive.buffers.length}`,
    );
    ...
    console.log(
      `[BENCHMARK] Optimized JSON size: ${(workerResultOptimized.json.length / 1024 / 1024).toFixed(2)} MB`,
    );
    console.log(
      `[BENCHMARK] Optimized Buffers count: ${workerResultOptimized.buffers.length}`,
    );

    expect(totalOptimized).toBeLessThan(totalNaive);
    expect(workerResultOptimized.json.length).toBeLessThan(
      workerResultNaive.json.length,
    );
