...
describe("PhysicsManager removeBody Performance", () => {
  ...
  test("Benchmark removal strategies (10,000 items)", () => {
    ...
    for (let i = bodiesToRemove.length - 1; i > 0; i--) {
      ...
      [bodiesToRemove[i], bodiesToRemove[j]] = [
        bodiesToRemove[j],
        bodiesToRemove[i],
      ];
    }
    ...
    for (const bodyToRemove of bodiesToRemove) {
      worldRemoveBodyMock(bodyToRemove);
      const index = bodiesSplice.findIndex(
        (item) => item.body === bodyToRemove,
      );
      if (index !== -1) {
    ...
    for (const bodyToRemove of bodiesToRemove) {
      worldRemoveBodyMock(bodyToRemove);
      const index = bodiesSwapPop.findIndex(
        (item) => item.body === bodyToRemove,
      );
      if (index !== -1) {
    ...
    for (let i = pmBodiesToRemove.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [pmBodiesToRemove[i], pmBodiesToRemove[j]] = [
        pmBodiesToRemove[j],
        pmBodiesToRemove[i],
      ];
    }
