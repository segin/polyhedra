...
export class PrimitiveFactory {
  ...
  _createMesh(geometry, color, side = THREE.FrontSide) {
    ...
    if (!isTest && !this.materialCache[cacheKey]) {
      this.materialCache[cacheKey] = new THREE.MeshStandardMaterial({
        color,
        side,
        roughness: 0.5,
        metalness: 0.1,
  ...
  createPrimitive(type, options = {}) {
    ...
    if (geometry) {
      mesh = this._createMesh(
        geometry,
        color,
        type === "Plane" ? THREE.DoubleSide : THREE.FrontSide,
      );
    } else {
      ...
      switch (type) {
        ...
        case "Teapot":
          ...
          teapot.add(
            this._createMesh(
              this._getCachedGeometry("Sphere", {
                radius: 0.4,
                widthSegments: 32,
                heightSegments: 32,
              }),
          ...
          const spout = this._createMesh(
            this._getCachedGeometry("Cylinder", {
              radiusTop: 0.05,
              radiusBottom: 0.08,
              height: 0.3,
              radialSegments: 8,
          ...
          const handle = this._createMesh(
            this._getCachedGeometry("Torus", {
              radius: 0.15,
              tube: 0.03,
              radialSegments: 8,
              tubularSegments: 16,
        ...
        case "LODCube":
          ...
          lod.addLevel(
            new THREE.Mesh(
              this._getCachedGeometry("Box", { width: 1, height: 1, depth: 1 }),
              lodMaterial,
            ),
            0,
          );
